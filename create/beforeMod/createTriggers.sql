...
-- Checks if the history has ended.
	IF (OLD.endHProject IS NOT NULL) THEN 
		SET msg = concat('Error: You can not change these datas because this project state has end date.');
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = msg;
	END IF;
...
-- ------------------------------- tPhaseInsert -------------------------------------
-- ----------------------------------------------------------------------------------
DROP TRIGGER IF EXISTS tPhaseInsert;

DELIMITER $$
CREATE TRIGGER tPhaseInsert BEFORE INSERT ON Phase 
FOR EACH ROW BEGIN
	DECLARE msg VARCHAR(128);

-- Checks the ini date
	CALL compatibleWithIniProject(NEW.idProject, NEW.iniPlanPhase, 'iniPlanPhase');
	
-- Checks the end date
	SET msg = concat('Error: Invalid value for endPlanPhase (',cast(NEW.endPlanPhase as char),'). It must be greater than iniPlanPhase.');
	CALL dateGreaterThan(NEW.iniPlanPhase,NEW.endPlanPhase,msg);
...
-- ------------------------------- tPhaseUpdate -------------------------------------
-- ----------------------------------------------------------------------------------
DROP TRIGGER IF EXISTS tPhaseUpdate;

DELIMITER $$
CREATE TRIGGER tPhaseUpdate BEFORE UPDATE ON Phase 
FOR EACH ROW BEGIN
	DECLARE msg VARCHAR(128);
	DECLARE aux INTEGER DEFAULT 0;

-- Checks the ini date
	SET aux = (SELECT COUNT(idTask)
			   FROM Task
			   WHERE idPhase = OLD.idPhase AND iniPlanTask < NEW.iniPlanPhase);

	IF (aux > 0) THEN
		SET msg = concat('Error: There are task with iniPlan smaller than this new iniPlanPhase.');
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = msg;
	END IF;
...
-- Checks the end date
	...
	SET aux = (SELECT COUNT(idTask)
			   FROM Task
			   WHERE idPhase = OLD.idPhase AND endPlanTask > NEW.endPlanPhase);

	IF (aux > 0) THEN
		SET msg = concat('Error: There are task with endPlan greater than this new endPlanPhase.');
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = msg;
	END IF;
...
-- Checks that milestone date is equal or greater than project ini date.
	...
	CALL compatibleWithPhase(NEW.idPhase, NEW.datePlanMilestone, NEW.datePlanMilestone, 'datePlanMilestone', 'datePlanMilestone');
...
-- Checks the planned and real datas. We don't check the daysPlanTask and iniPlanTask.
	SET thereAreInvalidData = (NEW.daysRealTask IS NOT NULL OR NEW.iniRealTask IS NOT NULL OR NEW.hoursPlanTask IS NOT NULL OR NEW.hoursRealTask IS NOT NULL OR NEW.endPlanTask IS NOT NULL OR NEW.endRealTask IS NOT NULL OR NEW.costPlanTask IS NOT NULL OR NEW.costRealTask);

	IF (thereAreInvalidData) THEN 
		SET msg = concat('Error: You can not insert daysRealTask, iniRealTask, the end dates, the hours or the costs.');
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = msg;
	END IF;
	...
	-- If iniPlanTask is defined, checks if is correct and establish endPlanTask.
		IF (NEW.iniPlanTask IS NOT NULL) THEN
			CALL compatibleWithIniProject(project, NEW.iniPlanTask, 'iniPlanTask');
			CALL compatibleWithPhase(NEW.idPhase, NEW.iniPlanTask, NEW.endPlanTask, 'iniPlanTask', 'endPlanTask');
		END IF;
	END IF;

	IF (NEW.idState='EJEC') THEN
		IF (NEW.daysPlanTask IS NOT NULL) THEN
			SET msg = concat('Error: You can not insert iniPlanTask or daysPlanTask.');
			SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = msg;
		END IF;
	END IF;
...
FOR EACH ROW BEGIN
	...
	DECLARE hoursP, costP, costM INTEGER DEFAULT 0;
	DECLARE project BIGINT;
	DECLARE projectState VARCHAR(4);
	DECLARE msg VARCHAR(128);
...
-- Booleans with the attribute changes.
	SET changeBeforeInsertPlan = (NEW.endPlanTask IS NOT NULL OR NEW.hoursPlanTask IS NOT NULL OR NEW.costPlanTask IS NOT NULL);
...
-- ----------------------------------------------------------------------------------
	...
	-- Recalculate endPlan if iniPlan or daysPlan were changed.
		...
		IF (thereAreChanges) THEN
			...
			CALL compatibleWithPhase(NEW.idPhase, NEW.iniPlanTask, (NEW.iniPlanTask + INTERVAL NEW.daysPlanTask DAY),
								 	 'iniPlanTask', 'endPlanTask');
		END IF;
	...
	-- If the new state is 'PRPD' we must check the next.
		...
		-- Calculate the costs and total hours.
			...
			SET NEW.endPlanTask = DATE_ADD( NEW.iniPlanTask, INTERVAL NEW.daysPlanTask DAY);
