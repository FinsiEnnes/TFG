...
BEGIN
	...
	DECLARE cur1 CURSOR FOR SELECT iniPlanTask, idState FROM Task WHERE idPhase IN (SELECT idPhase FROM Phase WHERE idProject = project);
...
BEGIN
	...
	IF ((endMax IS NULL) OR (ennd > endMax)) THEN 
		SET endMax = ennd;
	END IF;
	END LOOP;
...
BEGIN
	...
	DECLARE endPlan DATE DEFAULT NULL;
...
-- ----------------------------------------------------------------------------------
DROP PROCEDURE IF EXISTS updateProjectProgress;

DELIMITER $$
CREATE PROCEDURE updateProjectProgress(IN project BIGINT, IN task BIGINT, IN progressT INTEGER)
BEGIN
	DECLARE nTask, totalProgress, progressP INTEGER DEFAULT 0;

	SET nTask = (SELECT COUNT(idTask) FROM Task WHERE (idPhase IN (SELECT idPhase FROM Phase WHERE idProject = project)));
	SET totalProgress = (SELECT IFNULL(SUM(progressTask),0)
						FROM Task 
						WHERE (idTask != task)
						  AND (idPhase IN (SELECT idPhase FROM Phase WHERE idProject = project)));	

	SET totalProgress = totalProgress + progressT;

	-- Calculate the project progress
	SET progressP = (totalProgress / nTask);

	-- Update the project progress
	UPDATE Project
	SET progressProject = progressP
	WHERE idProject = project;
...
-- ------------------------------ updateIniPhase ------------------------------------
-- ----------------------------------------------------------------------------------
-- Check if it is necessary a refresh of the ini date Phase.
-- ----------------------------------------------------------------------------------
DROP PROCEDURE IF EXISTS updateIniPhase;

DELIMITER $$
CREATE PROCEDURE updateIniPhase(IN project BIGINT, IN phase BIGINT, IN newTaskIniDate DATE)
BEGIN
	DECLARE currentState VARCHAR(4);
	DECLARE currentMinIni, iniPhaseDate DATE;
...
-- Get the ini date of the Phase
	SET iniPhaseDate = (SELECT iniPhase FROM Phase WHERE idPhase=phase);

-- In function of the state we select a date or other
	IF (currentState='PLAN') THEN 
		SET currentMinIni = (SELECT MIN(iniPlanTask) FROM Task WHERE idPhase=phase);
	ELSEIF (currentState='EJEC') THEN
		SET currentMinIni = (SELECT MIN(iniRealTask) FROM Task WHERE idPhase=phase); 
	END IF;

-- Update the iniPhase if...
	IF ((currentMinIni > newTaskIniDate) OR (currentMinIni IS NULL)) THEN 
		UPDATE Phase SET iniPhase=newTaskIniDate where idPhase=phase;
	END IF;
...
-- ------------------------------ updateEndPhase ------------------------------------
-- ----------------------------------------------------------------------------------
-- Check if it is necessary a refresh of the end date Phase.
-- ----------------------------------------------------------------------------------
DROP PROCEDURE IF EXISTS updateEndPhase;

DELIMITER $$
CREATE PROCEDURE updateEndPhase(IN project BIGINT, IN phase BIGINT, IN newTaskEndDate DATE)
BEGIN
	DECLARE currentState VARCHAR(4);
	DECLARE currentMaxEnd, endPhaseDate DATE;
...
-- Get the ini date of the Phase
	SET endPhaseDate = (SELECT endPhase FROM Phase WHERE idPhase=phase);

-- In function of the state we select a date or other
	IF (currentState='PLAN') THEN 
		SET currentMaxEnd = (SELECT MAX(endPlanTask) FROM Task WHERE idPhase=phase);
	ELSEIF (currentState='EJEC') THEN
		SET currentMaxEnd = (SELECT MAX(endRealTask) FROM Task WHERE idPhase=phase); 
	END IF;

-- Update the iniPhase if...
	IF ((currentMaxEnd < newTaskEndDate) OR (currentMaxEnd IS NULL)) THEN 
		UPDATE Phase SET endPhase=newTaskEndDate where idPhase=phase;
	END IF;
